  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        {/* Occasions grid — 2 cols on sm, 3 cols on lg */}
        <StaggerChildren staggerDelay={0.1} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 max-w-6xl mx-auto">
          {OCCASIONS.map((occasion) => (
            <StaggerItem key={occasion.slug}>
              <Link
                href={`/occasions/${occasion.slug}`}
                className="group relative block aspect-3/4 rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-300 hover:-translate-y-1"
              >
                {/* Photo background */}
                <Image
                ...
                <div className="absolute inset-x-0 bottom-0 p-5 md:p-6 flex flex-col justify-end">
                  ...
                  <p className="text-sm md:text-base text-white/70 font-light leading-relaxed mb-3 transition-all duration-300 group-hover:text-white/90">
