  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => {
        const isFilled = rating >= i + 1 || (!Number.isInteger(rating) && rating >= i + 0.5);
        return (
          <Star
            key={i}
            className={`${size} ${
...
export function ReviewsCarousel({ rating, totalReviews, googleUrl }: ReviewsCarouselProps) {
    ...
    <section id="reviews" className="relative py-24 overflow-hidden">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-6 lg:px-8">
          ...
          <div className="relative rounded-2xl border border-border/60 bg-card/80 backdrop-blur-sm px-10 py-8 text-center shadow-lg shadow-gold/5">
            ...
            {googleUrl && (
              <a
                ...
                className="inline-flex items-center gap-1.5 text-sm text-gold hover:text-gold-light transition-colors duration-300"
              >
                {t("seeAllReviews")}
        ...
        >
          ...
          <CarouselPrevious className="hidden sm:inline-flex left-0 md:-left-2 size-10 border-border/60 bg-card/80 hover:bg-gold/10 hover:border-gold/40 hover:text-gold" />
          <CarouselNext className="hidden sm:inline-flex right-0 md:-right-2 size-10 border-border/60 bg-card/80 hover:bg-gold/10 hover:border-gold/40 hover:text-gold" />
        </Carousel>
      </div>
    </section>
  );
